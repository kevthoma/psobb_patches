...
| `RightStickOrbitWhileHolding` | `1` | Keep that pivot on your character while the camera is merely **holding** an angle, not only while you steer. Without it, running with the camera behind you in ChaseCam Disabled placed the camera around the look-ahead point on the far side of your character, so it sat far too close: median 27 units while running (17-20% of running time under 20), against about 60 for an ordinary chase camera. |
