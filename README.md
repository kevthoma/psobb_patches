...
| `RightStickYieldToFocusCamera` | `1` | Hand the camera back while the client has taken it for itself -- the close-up during a focused NPC conversation. Detected by the client zeroing its look-at lerp (`+0x1B8`), which was the only field in the camera state that separated conversation from play. The angle you held beforehand is resumed afterwards. |
